...
import { Server as Dropper } from "https://deno.land/x/dropper@1.10.0/src/mod.ts";
...
export default class Livereload {
  ...
  constructor(
    arg:
      | Array<string>
      | string
      | Record<string, string | number | Array<string> | boolean>,
  ) {
  ...
  private async init() {
    ...
    const filePath = Array.isArray(this.#options.base)
      ? this.#options.base.map((e: string) => resolve(Deno.cwd(), e))
      : resolve(Deno.cwd(), this.#options.base);
    ...
    for await (const ev of watcher) {
      ...
      if (
        this.#options?.exclude?.some((glob: string) =>
          globToRegexp(glob).test(normalize(ev.paths[0]))
        )
      ) continue;
